...
import { LayoutDashboard, Package, ShoppingCart, BarChart3, LogOut, Bell, User, Search, Store, Menu, X } from 'lucide-react';
        ...
        <div className="w-full px-6 lg:px-12 flex justify-between items-center gap-8">
          ...
          <Link href="/" className="flex items-center gap-4 group">
            <div className={cn(
              ...
              isScrolled ? "bg-white/5 border border-white/10" : "bg-emerald-500/20 border border-emerald-500/30 group-hover:scale-110"
            )}>
          ...
          <div className="flex items-center gap-4">
            <div className="hidden lg:block relative group">
              ...
              />
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-white/30 group-focus-within:text-emerald-500 transition-colors" size={16} />
              <div className="absolute right-3 top-1/2 -translate-y-1/2 px-1.5 py-0.5 rounded-md bg-white/10 text-[8px] font-black opacity-50">⌘K</div>
            </div>
            ...
            <button className="w-12 h-12 flex items-center justify-center bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 transition-all relative">
              <Bell size={18} className="text-white/60 hover:text-emerald-500" />
          ...
          >
            <div className="flex justify-between items-center mb-16">
              <div className="flex flex-col">
                <span className="text-3xl font-black italic tracking-tighter text-emerald-500 leading-none">AURORA</span>
                <span className="text-[10px] font-black uppercase tracking-[0.5em] text-emerald-500/50 italic mt-1 relative left-[2px]">Merchant Node</span>
              </div>
              <button 
                  ...
                  >
                    ...
                    >
                      <div className={cn(
                         ...
                         isActive ? "bg-emerald-500/20 border-emerald-500/30" : "bg-white/5 border-white/5 group-hover:bg-emerald-500/10 group-hover:border-emerald-500/20"
                      )}>
                        <Icon size={20} className={cn(
                          "transition-all",
                          isActive ? "text-emerald-500" : "text-white/40 group-hover:text-emerald-500 group-hover:translate-x-2"
                        )} />
                      </div>
                      <span className={cn(
                        "text-2xl font-black uppercase tracking-widest transition-colors",
                        isActive ? "text-emerald-500" : "text-white/60 group-hover:text-white"
                      )}>
                        {link.label}
                      </span>
                    </Link>
